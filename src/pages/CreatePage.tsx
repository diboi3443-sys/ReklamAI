  ...
  useEffect(() => {
    async function loadModels() {
      ...
      try {
        ...
        console.log('[CreatePage] Loading models for preset:', selectedPreset, 'modality:', modality);

        const models = await loadModelsByModality(modality);
        console.log('[CreatePage] Loaded models:', models.length);

        setAvailableModels(models);
        console.log('[CreatePage] Available models set:', models.length, 'models');
        console.log('[CreatePage] Current selectedModel:', selectedModel);
        ...
        if (models.length > 0) {
          ...
          console.log('[CreatePage] Current model exists in list:', currentModelExists);
          if (!currentModelExists || !selectedModel) {
            const firstModelKey = models[0].key;
            console.log('[CreatePage] Auto-selecting first model:', firstModelKey, models[0].title);
            setSelectedModel(firstModelKey);
          } else {
            console.log('[CreatePage] Keeping current model:', selectedModel);
          }
        } else {
          console.warn('[CreatePage] No models available to select');
          setSelectedModel("");
        }
      } catch (error) {
        console.error('[CreatePage] Error loading models:', error);
        setAvailableModels([]);
      } finally {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => {
                          console.log('[CreatePage] Model selected:', value);
                        ...
                        <SelectContent>
                          {modelsLoading ? (
                            ...
                            availableModels.map((model) => {
                              ...
                              console.log('[CreatePage] Rendering model option:', model.key, model.title, 'selected:', isSelected);
                              return (
                ...
                <div className="flex items-center gap-2">
                  {currentBoard && (
                    <Badge variant="outline" className="gap-1.5">
                      ...
                      {t.boards.saveToBoard}: {currentBoard.name}
                    </Badge>
                  )}
                  <Button
                    variant="glow"
