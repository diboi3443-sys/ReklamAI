...
/**
 * Table Component - ReklamAI Design System
 * 
 * Design tokens:
 * - Row height: h-11 (44px) for header, flexible for body
 * - Cell padding: px-4 py-3
 * - Border: 1px solid border on rows
 * - Hover: bg-secondary/40 for subtle feedback
 */
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        "border-b border-border transition-colors data-[state=selected]:bg-muted hover:bg-secondary/30",
        className
      )}
      {...props}
    />
